...
# load data
politicians <- fromJSON("C:\\Users\\shado\\Documents\\Abgeordnete_Kommunikation\\abgeordnetenwatch_data.json")
```
...
for (id in 1:length(politicians$ID)) {
  ...
  if (is.null(dat$data$year_of_birth)) {
    ...
    year_of_birth <- append(year_of_birth, dat$data$year_of_birth)
  }
...
```{r}
# export updated data
politicians <- toJSON(politicians, pretty = TRUE)

write(json_data, file = "abgeordnetenwatch_data.json")
```
